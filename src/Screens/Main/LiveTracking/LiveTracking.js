...
const LiveTracking = ({ route }) => {
    ...
    if (
      ...
      if (ongoingRideData?.onBoardPassengers) {
        if (
          ...
          //console.log("directionApiResp", "inside");
          Geolocation.getCurrentPosition(
            (position) => {
              let driverLastPointCoords = {
          ...
          NetInfo.fetch().then((state) => {
            ...
            if (networkStatus) {
              ...
              getItem("reGenneratePolyline").then((res) => {
                //console.log("DirectionApiResp", res);
                if (res == null) {
                  reGenerateRoute();
                }
              });
  ...
  const callSpeedLimitApi = () => {
    // let sendingData = `OverSpeed/${profileData?.id}`;
    // actions
    //   .callSpeedLimitApi(sendingData)
    //   .then((res) => {})
    //   .catch((err) => {
    //     setisLoading(false);
    //   });
  ...
  const getSimnumbers = () => {
    SimCardsManagerModule.getSimCards({
      title: "eTravelMate Driver",
      message: "eTravelMate want to access your app for calling.",
